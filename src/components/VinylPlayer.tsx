...
export const VinylPlayer: React.FC<VinylPlayerProps> = ({ isVisible, onClose }) => {
  console.log('🎭 VinylPlayer rendered with isVisible:', isVisible);
  ...
  const startVinylRotation = () => {
    ...
    Animated.loop(
      Animated.timing(vinylRotation, {
        toValue: 1,
        duration: 3000, // 3 seconds per rotation (33 RPM effect)
        useNativeDriver: true,
      })
    ).start();
  };
  ...
  const getStationLogo = (stationName: string) => {
    ...
    if (stationName.toLowerCase().includes('kar')) return '❄️';    return '📻';
  };

  if (!isVisible) {
    console.log('🚫 VinylPlayer - Not visible, returning null');
    return null;
  }

  if (!playbackState.currentStation) {
    console.log('🚫 VinylPlayer - No current station but modal is visible');
    // Return empty modal with loading state
    return (
      <Modal visible={isVisible} animationType="fade" transparent>
        <BlurView intensity={50} style={styles.overlay}>
          <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
            <Text style={{ color: '#FFD700', fontSize: 18 }}>🎵 RADYO ÇINARI</Text>
            <Text style={{ color: '#FFD700', fontSize: 14, marginTop: 10 }}>Radyo yükleniyor...</Text>
            <ActivityIndicator size="large" color="#FFD700" style={{ marginTop: 20 }} />
            <TouchableOpacity onPress={onClose} style={{ marginTop: 30, padding: 15 }}>
              <Text style={{ color: '#FFD700' }}>Kapat</Text>
            </TouchableOpacity>
          </View>
        </BlurView>
      </Modal>
  ...
  console.log('✅ VinylPlayer - Rendering with station:', playbackState.currentStation.name);
